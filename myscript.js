...
function store()
{
			...
			for (var i = 0; i < cart_items.length; i++) 
			    {
						var cart_item = cart_items[i];
						//var prodtnm = cart_item.getElement('id');
						var prodtnm = cart_items[i].childNodes[0].innerText;
						var quantity = cart_item.getAttribute('data-quantity');
						var price = cart_item.getAttribute('data-price');

						var sub_total = parseFloat(quantity * parseFloat(price));					

			     products.push({name:prodtnm,quantity:quantity,price:price,subtotal:sub_total});
			   }
